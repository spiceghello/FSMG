...
Axiom dlbr : forall (n : nat) (a : deloop n),
  ap dli (dltw a) @ dltw (dli a) @ ap dli (dltw a) = dltw (dli a) @ ap dli (dltw a) @ dltw (dli a)
...
Definition dlbr'_rec_type
  ...
  : Type
  := forall (n : nat) (a' : P n),
    ap (dli' _) (dltw' _ a') @ dltw' _ (dli' _ a') @ ap (dli' _) (dltw' _ a') = dltw' _ (dli' _ a') @ ap (dli' _) (dltw' _ a') @ dltw' _ (dli' _ a').
...
Context (P : Type)
  ...
  (dlbr' : forall a' : P, (ap dli' (dltw' a') @ dltw' (dli' a')) @ ap dli' (dltw' a') = (dltw' (dli' a') @ ap dli' (dltw' a')) @ dltw' (dli' a'))
  (dlT' : IsTrunc 1 P).
...
Lemma deloop_rec_truncfib
  ...
  (dlbr' : forall (n : nat) (a : A), (ap (dli' n.+2) (dltw' n a) @ dltw' n.+1 (dli' n a)) @ ap (dli' n.+2) (dltw' n a) = (dltw' n.+1 (dli' n a) @ ap (dli' n.+2) (dltw' n a)) @ dltw' n.+1 (dli' n a))
...
Defined.
Global Arguments dlfa {n} {a} {b} p.

Lemma dlfa_n
  : forall (n : nat) (a b : deloop n) (p : a = b),
  ap (sigma_function S (@dli)) (ap (sigma_function S (@dli)) (path_sigma' deloop idpath p)) @ path_sigma' deloop idpath (dltw b) = path_sigma' deloop idpath (dltw a) @ ap (sigma_function S (@dli)) (ap (sigma_function S (@dli)) (path_sigma' deloop idpath p)).
Proof.
  intros; induction p; simpl.
  exact (concat_1p _ @ (concat_p1 _)^).
Defined.
Global Arguments dlfa_n {n} {a} {b} p.
...
(** Symmetric monoidal structure on del_dot **)
...
Context `{Univalence}.
  ...
  Lemma dlbr_dot
    : forall a : deloop_dot,
    ap Sdli (dltw_dot a) @ dltw_dot (Sdli a) @ ap Sdli (dltw_dot a) = dltw_dot (Sdli a) @ ap Sdli (dltw_dot a) @ dltw_dot (Sdli a).
  Proof.
    intro.
    unfold Sdli.
    ...
    change ((ap (sigma_function S (@dli)) (path_sigma' deloop 1 (dltw a.2)) @ path_sigma' deloop 1 (dltw (sigma_function S (@dli) a).2)) @ ap (sigma_function S (@dli)) (path_sigma' deloop 1 (dltw a.2)) = (path_sigma' deloop 1 (dltw (sigma_function S (@dli) a).2) @ ap (sigma_function S (@dli)) (path_sigma' deloop 1 (dltw a.2))) @ path_sigma' deloop 1 (dltw (sigma_function S (@dli) a).2)).
...
Lemma ap011_deloop_m_ap_cons
  {a a' b b' : deloop_dot} (p : a = a') (q : b = b')
  : ap011 deloop_m (ap (sigma_function S (@dli)) p) q = ap (sigma_function S (@dli)) (ap011 deloop_m p q).
Proof.
  induction p, q; constructor.
Defined.

Definition deloop_alp
...
Proof.
  ...
  + abstract (intros n a a'; simpl;
    ...
    apply dlfa_dot ) using deloop_alp_dltw.
Defined.

Definition deloop_lam
  : forall b : deloop_dot, deloop_m deloop_e b = b.
Proof.
  constructor.
Defined.

Definition deloop_rho
...
Proof.
  ...
  + abstract (intros n a a'; simpl;
    ...
    exact (dlfa_dot a') ) using deloop_rho_dltw.
Defined.

    Lemma deloop_tau_Q_dltw (y : deloop_dot)
    ...
    Proof.
      ...
      refine (whiskerL _ (dlfa_dot _) @ concat_p_pp _ _ _ @ _); apply whiskerR.
      symmetry; apply dlbr_dot.
    Defined.

  Definition deloop_tau_Q
  ...
  Proof.
    ...
    + intros n a h. change (Sdli (Sdli (deloop_m (n; a) y)) = Sdli (deloop_m (n; a) (Sdli y))).
      exact (dltw_dot _ @ (ap Sdli h)).
    + apply deloop_tau_Q_dltw.
  Defined.

  Definition deloop_tau_R
    : forall (n : nat) (a : deloop n) (b : deloop_dot),
      ap Sdli (deloop_tau_Q (n; a) b) @ (deloop_tau_Q (n.+1; dli a) b @ ap (fun a0 : deloop n.+2 => deloop_m b (n.+2; a0)) (dltw a))
      = dltw_dot (deloop_m b (n; a)) @ (ap Sdli (deloop_tau_Q (n; a) b) @ deloop_tau_Q (n.+1; dli a) b).
  Proof.
  Admitted. (* it works the same way as for slists *)

  Lemma deloop_tau_dltw
    : forall (n : nat) (a : deloop n) (a' : forall b : {n0 : nat & deloop n0}, deloop_m (n; a) b = deloop_m b (n; a)),
      transport
        ...
        (dltw a)
        (fun b : {n0 : nat & deloop n0} => ap Sdli (ap Sdli (a' b) @ deloop_tau_Q (n; a) b) @ deloop_tau_Q (n.+1; dli a) b)
      = (fun b : {n0 : nat & deloop n0} => ap Sdli (ap Sdli (a' b) @ deloop_tau_Q (n; a) b) @ deloop_tau_Q (n.+1; dli a) b).
  Proof.
    intros.
    ...
    rewrite <- dlfa_dot; repeat rewrite concat_pp_p; apply whiskerL.
    srapply deloop_tau_R.
  Defined.

Definition deloop_tau
...
Proof.
  ...
  srefine (deloop_ind_to_set _ _ _ _ T); clear T; hnf.
  + intro. change (b = deloop_m b (0; dlpt0)). exact (deloop_rho b)^.
  + intros n a ih b.
    change (Sdli (deloop_m (n; a) b) = deloop_m b (Sdli (n; a))).
    refine (ap Sdli (ih b) @ _).
    exact (deloop_tau_Q (n; a) b).
  + exact deloop_tau_dltw.
Defined.

(* This goes exactly the same way as for slists.
We show the pentagon as an example. *)
Definition deloop_pentagon
  : IsPentagonCoherent deloop_m deloop_alp.
Proof.
  unfold IsPentagonCoherent.
  intros [n a] b c d; revert a; revert n.
  assert (T : forall (n : nat) (a : deloop n), IsHProp (deloop_alp (deloop_m (n; a) b) c d @ deloop_alp (n; a) b (deloop_m c d) =
(ap011 deloop_m (deloop_alp (n; a) b c) 1 @ deloop_alp (n; a) (deloop_m b c) d) @
ap011 deloop_m 1 (deloop_alp b c d))).
  ...
  + simpl. refine (concat_p1 _ @ _ @ (concat_1p _)^).
    change (deloop_alp b c d = ap idmap (deloop_alp b c d)).
    exact (ap_idmap _)^.
  + intros n a h.
    change (ap (sigma_function S (@dli)) (deloop_alp (deloop_m (n; a) b) c d) @ ap (sigma_function S (@dli)) (deloop_alp (n; a) b (deloop_m c d)) = (ap011 deloop_m (ap (sigma_function S (@dli)) (deloop_alp (n; a) b c)) 1 @ ap (sigma_function S (@dli)) (deloop_alp (n; a) (deloop_m b c) d)) @ ap011 deloop_m (ap (sigma_function S (@dli)) (idpath (n; a))) (deloop_alp b c d)).
    rewrite (ap011_deloop_m_ap_cons (deloop_alp (n; a) b c) (idpath d)).
    rewrite (ap011_deloop_m_ap_cons (idpath (n; a)) (deloop_alp b c d)).
    repeat rewrite <- (ap_pp ((sigma_function S (@dli)))); apply ap; exact h.
Qed.

Definition deloop_triangle
  : IsTriangleCoherent deloop_e deloop_m deloop_alp deloop_lam deloop_rho.
Proof.
Admitted.

Definition deloop_hexagon
  : IsHexagonCoherent deloop_m deloop_alp deloop_tau.
Proof.
Admitted.

Definition deloop_bigon
  : IsBigonCoherent deloop_m deloop_tau.
Proof.
Admitted.

Definition deloop_smgpd
  : SymMonoidalGroupoid
  := Build_SymMonoidalGroupoid deloop_dot dlT_dot deloop_e deloop_m deloop_alp deloop_lam deloop_rho deloop_tau deloop_pentagon deloop_triangle deloop_hexagon deloop_bigon.
